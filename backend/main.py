...
async def esp32_out_upload(rfid_id: str = Query(...), request: Request = None):
    ...
    try:
        ...
        plate_text = None
        try:
            # Use the same detection logic as in-upload
            plate_text = fastalpr_read_plate(alpr, img, str(save_path))
        except Exception as alpr_error:
            print(f"FastALPR failed: {alpr_error}")
            plate_text = None
        ...
        try:
            ...
            if not detected_plate_clean:
                raise HTTPException(
                    status_code=400,
                    detail="Could not detect license plate from image. Vehicle cannot exit.",
                )
            ...
            if not stored_plate_clean:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"License plate mismatch. Detected: {plate_text}, Expected: {stored_plate}. Vehicle cannot exit.",
                )
